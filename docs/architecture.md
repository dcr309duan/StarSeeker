...
- Node/Express 服务：`server.js`，端口 `3000`，提供图片识别与解析、历史与收藏接口，并托管静态页面。
- Kotlin/Spring Boot 服务：`backend/`，端口 `8080`，提供版本化 REST API（`/v1/api`），集成 JWT 与 Swagger，兼容解析流程（文本或图片）。
- 题库与索引：当前均为内存实现，Node 侧使用 `lunr` 建索引；Spring 侧在 `QuestionRepository` 中维护示例题。
...
- Node/Express
  - 入口：`server.js:100-102` 启动服务与端口日志；静态托管 `server.js:89`。
  - 识别主流程：`server.js:22-72`（上传→OCR→解析→检索→生成解析→写入历史→返回响应）。
